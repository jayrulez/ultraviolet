...
namespace Sedulous.BASS
{
    ...
    /// </summary>
    public sealed unsafe class BASSSedulousAudio : SedulousResource, ISedulousAudio, IMessageSubscriber<SedulousMessageID>
    {
        /// <summary>
        /// Initializes a new instance of the BASSSedulousAudio class.
        /// </summary>
        /// <param name="uv">The Sedulous context.</param>
        public BASSSedulousAudio(SedulousContext uv)
            : base(uv)
        {
            this.Capabilities = new BASSAudioCapabilities();

            if (uv.Platform == SedulousPlatform.Windows || uv.Platform == SedulousPlatform.macOS)
            {
        ...
        public ISedulousAudioDevice PlaybackDevice
        {
            ...
            set
            {
                ...
                if (val != PlaybackDevice)
                {
                    ...
                    {
                        if (val is BASSSedulousAudioDevice device)
                        {
                            ...
                            if (Sedulous != null && !Sedulous.Disposed)
                            {
                                ...
                                Sedulous.Messages.PublishImmediate(BASSSedulousMessages.BASSDeviceChanged, data);
                            }
        ...
        private void UpdateAudioDevices()
        {
            ...
            for (int i = 1; BASS_GetDeviceInfo((uint)i, &info); i++)
            {
                ...
                if (ix >= knownAudioDevices.Count)
                {
                    ...
                    knownAudioDevices.Add(new BASSSedulousAudioDevice(Sedulous, (uint)i, marshalledInfo.name));
                }
        ...
        /// </summary>
        private BASSSedulousAudioDevice GetDefaultDevice()
        {
            foreach (var device in knownAudioDevices)
            {
        ...
        // Audio device cache.
        private ISedulousAudioDevice playbackDevice;
        private List<BASSSedulousAudioDevice> knownAudioDevices = 
            new List<BASSSedulousAudioDevice>();
    }
}
