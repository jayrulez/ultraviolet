...
namespace Sedulous.Shims.iOS.Platform
{
    ...
    public sealed class iOSScreenDensityService : ScreenDensityService
    {
        ...
        public unsafe iOSScreenDensityService(IFrameworkDisplay display)
            : base(display)
        {
            deviceScale = (Single)UIScreen.MainScreen.Scale;

            switch (FrameworkPlatformInfo.CurrentPlatformMachineHardwareName)
            {
                // Simulator
                case "i386":
                case "x86_64":
            ...
            var displayIsScaled = !FrameworkContext.DemandCurrent().Properties.SupportsHighDensityDisplayModes;
            if (displayIsScaled)
            {
