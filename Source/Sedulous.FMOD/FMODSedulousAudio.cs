...
namespace Sedulous.FMOD
{
    ...
    /// </summary>
    public sealed unsafe partial class FMODSedulousAudio : SedulousResource, ISedulousAudio, IMessageSubscriber<SedulousMessageID>
    {
        /// <summary>
        ...
        public FMODSedulousAudio(SedulousContext uv, SedulousConfiguration configuration)
            : base(uv)
        {
            ...
            uv.Messages.Subscribe(this, SedulousMessages.ApplicationCreated);
            uv.Messages.Subscribe(this, SedulousMessages.ApplicationTerminating);
            uv.Messages.Subscribe(this, SedulousMessages.ApplicationSuspending);
            uv.Messages.Subscribe(this, SedulousMessages.ApplicationResumed);
            uv.Messages.Subscribe(this, SedulousMessages.FileSourceChanged);
        }

        /// <inheritdoc/>
        void IMessageSubscriber<SedulousMessageID>.ReceiveMessage(SedulousMessageID type, MessageData data)
        {
            if (type == SedulousMessages.ApplicationCreated)
            {
            ...
            if (type == SedulousMessages.ApplicationSuspending)
            {
                if (!suspended)
                {
                    awaitingResume = true;
            ...
            if (type == SedulousMessages.ApplicationResumed)
            {
                if (awaitingResume)
                {
            ...
            if (type == SedulousMessages.FileSourceChanged)
            {
                UpdateFileSource();
                return;
            }
        ...
        public ISedulousAudioDevice PlaybackDevice
        {
            ...
            set
            {
                ...
                {
                    if (val is FMODSedulousAudioDevice device)
                    {
        ...
        private static FMOD_RESULT DebugCallbackThunk(FMOD_DEBUG_FLAGS flags, String file, Int32 line, String func, String message)
        {
            ...
            if (SedulousContext.RequestCurrent()?.GetAudio() is FMODSedulousAudio audio)
        ...
        /// </summary>
        private void InitializeLogging(SedulousConfiguration configuration)
        {
            var result = default(FMOD_RESULT);
        ...
        private void UpdateAudioDevices()
        {
            ...
            if (numdrivers != knownAudioDevices.Count)
            {
                ...
                for (var i = 0; i < numdrivers; i++)
                {
                    ...
                    var device = new FMODSedulousAudioDevice(Sedulous, i, namebuf.ToString())
                    {
                        IsValid = true,
                        IsDefault = i == 0
                    };
        ...
        /// </summary>
        private FMODSedulousAudioDevice GetDefaultDevice()
        {
        ...
        // Audio device cache.
        private FMODSedulousAudioDevice playbackDevice;
        private List<FMODSedulousAudioDevice> knownAudioDevices = new List<FMODSedulousAudioDevice>();
