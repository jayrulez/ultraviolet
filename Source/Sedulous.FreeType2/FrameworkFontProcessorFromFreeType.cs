...
namespace Sedulous.FreeType2
{
    ...
    public sealed class FrameworkFontProcessorFromFreeType : ContentProcessor<FreeTypeFontInfo, FrameworkFont>
    {
        ...
        /// <inheritdoc/>
        public override FrameworkFont Process(ContentManager manager, IContentProcessorMetadata metadata, FreeTypeFontInfo input) =>
        ...
        {
            var impl = FrameworkContext.DemandCurrent().GetContent().Processors.FindProcessor(typeof(FreeTypeFontInfo), typeof(FreeTypeFont));
            if (impl == null)
