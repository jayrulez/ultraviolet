...
namespace Sedulous
{
    ...
    public sealed class FrameworkFactory
    {
        ...
        public T GetFactoryMethod<T>() where T : class
        {
            ...
            if (value == null)
                throw new InvalidOperationException(FrameworkStrings.MissingFactoryMethod.Format(typeof(T).FullName));
        ...
        public T GetFactoryMethod<T>(String name) where T : class
        {
            ...
            var registry = default(Dictionary<String, Delegate>);
            if (!namedFactoryMethods.TryGetValue(typeof(T), out registry))
                throw new InvalidOperationException(FrameworkStrings.NoNamedFactoryMethods.Format(typeof(T).FullName));
            
            var value = default(Delegate);
            registry.TryGetValue(name, out value);

            if (value == null)
                throw new InvalidOperationException(FrameworkStrings.MissingNamedFactoryMethod.Format(name));
        ...
        public void SetFactoryMethod<T>(T factory) where T : class
        {
            ...
            if (del == null)
                throw new InvalidOperationException(FrameworkStrings.FactoryMethodInvalidDelegate);
            
            if (defaultFactoryMethods.ContainsKey(key))
                throw new InvalidOperationException(FrameworkStrings.FactoryMethodAlreadyRegistered);
        ...
        public void SetFactoryMethod<T>(String name, T factory) where T : class
        {
            ...
            if (registry.ContainsKey(name))
                throw new InvalidOperationException(FrameworkStrings.NamedFactoryMethodAlreadyRegistered);
