using System;
using Sedulous.Core;

namespace Sedulous
{
    /// <summary>
    /// Represents an object which encapsulates some native or implementation-specific resource.
    /// </summary>
    public abstract class SedulousResource : ISedulousComponent, IDisposable
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="SedulousResource"/> class.
        /// </summary>
        /// <param name="uv">The Sedulous context.</param>
        protected SedulousResource(SedulousContext uv)
        {
            Contract.Require(uv, nameof(uv));

            this.uv = uv;
        }

        /// <summary>
        /// Releases resources associated with the object.
        /// </summary>
        public void Dispose()
        {
            Dispose(true);
            GC.SuppressFinalize(this);
        }

        /// <summary>
        /// Gets the Sedulous context.
        /// </summary>
        public SedulousContext Sedulous
        {
            get { return uv; }
        }

        /// <summary>
        /// Gets a value indicating whether the object has been disposed.
        /// </summary>
        public Boolean Disposed
        {
            get { return disposed; }
        }

        /// <summary>
        /// Releases resources associated with the object.
        /// </summary>
        /// <param name="disposing"><see langword="true"/> if the object is being disposed; <see langword="false"/> if the object is being finalized.</param>
        protected virtual void Dispose(Boolean disposing)
        {
            disposed = true;
        }

        // Property values.
        private readonly SedulousContext uv;

        // State values.
        private Boolean disposed;
    }
}
