...
{
    /// <summary>
    /// Contains the Sedulous Presentation Foundation's string resources.
    /// </summary>
    public static class PresentationStrings
    {
        ...
        static PresentationStrings()
        {
            ...
            using (var stream = asm.GetManifestResourceStream("Sedulous.Presentation.Resources.Strings.xml"))
            {
                StringDatabase.LoadFromStream(stream);
            }
            using (var stream = asm.GetManifestResourceStream("Sedulous.Presentation.Resources.Strings_Commands.xml"))
            {
                StringDatabase.LoadFromStream(stream);
            }
        }
