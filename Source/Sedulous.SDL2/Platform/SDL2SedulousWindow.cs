...
{
    /// <summary>
    /// Represents the SDL2 implementation of the <see cref="ISedulousWindow"/> interface.
    /// </summary>    
    public sealed unsafe partial class SDL2SedulousWindow : SedulousResource,
        IMessageSubscriber<SedulousMessageID>,
        ISedulousWindow
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="SDL2SedulousWindow"/> class.
        /// </summary>
        ...
        internal SDL2SedulousWindow(SedulousContext uv, IntPtr ptr, Boolean visible, Boolean native = false)
            : base(uv)
        {
            this.ptr = ptr;
        ...
        public static explicit operator IntPtr(SDL2SedulousWindow window)
        {
                ...
                case WindowMode.Windowed:
                    {
                        ...
                        var x = windowedPosition?.X ?? SedulousConfiguration.DefaultWindowPositionX;
                        var y = windowedPosition?.Y ?? SedulousConfiguration.DefaultWindowPositionY;
                        var w = windowedClientSize?.Width ?? SedulousConfiguration.DefaultWindowClientWidth;
                        var h = windowedClientSize?.Height ?? SedulousConfiguration.DefaultWindowClientHeight;
                        ...
                        if (Sedulous.Platform == SedulousPlatform.Windows)
        ...
        public void ChangeCompositor(Compositor compositor)
        {
            ...
            if (compositor.Window != this)
                throw new InvalidOperationException(SedulousStrings.CompositorAssociatedWithWrongWindow);

            if (IsCurrentWindow)
                throw new InvalidOperationException(SedulousStrings.CannotChangeCompositorWhileCurrent);
        ...
        /// <inheritdoc/>
        public void ChangeDisplay(ISedulousDisplay display)
        {
            Contract.EnsureNotDisposed(this, Disposed);
        ...
        public void Draw(SedulousTime time)
        {
            OnDrawing(time);
            OnDrawingUI(time);
        }
        ...
        private static Surface2D LoadDefaultWindowIcon(SedulousContext uv)
        {
        ...
        /// </summary>
        /// <param name="time">Time elapsed since the last call to Draw.</param>
        private void OnDrawing(SedulousTime time) =>
        ...
        /// </summary>
        /// <param name="time">Time elapsed since the last call to Draw.</param>
        private void OnDrawingUI(SedulousTime time) =>
        ...
        private void HandleDpiChanged(Single? reportedScale = null)
        {
            ...
            ((SDL2SedulousDisplay)Display)?.RefreshDensityInformation();

            // On Windows, resize the window to match the new scale.
            if (Sedulous.Platform == SedulousPlatform.Windows && Sedulous.Properties.SupportsHighDensityDisplayModes)
            {
            ...
            Sedulous.Messages.Publish(SedulousMessages.WindowDensityChanged, messageData);
        }
        ...
        {
            if (Sedulous.Platform != SedulousPlatform.Windows || win32CachedStyle == IntPtr.Zero)
                return false;
        ...
        private static readonly SedulousSingleton<Surface2D> DefaultWindowIcon = new SedulousSingleton<Surface2D>(
