...
{
    /// <summary>
    /// Represents the SDL2 implementation of the <see cref="ISedulousWindowInfo"/> interface when
    /// using the OpenGL rendering API.
    /// </summary>
    public sealed class SDL2SedulousWindowInfoOpenGL : SDL2SedulousWindowInfo
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="SDL2SedulousWindowInfoOpenGL"/> class.
        /// </summary>
        /// <param name="uv">The Sedulous context.</param>
        /// <param name="configuration">The Sedulous configuration settings for the current context.</param>
        public SDL2SedulousWindowInfoOpenGL(SedulousContext uv, SedulousConfiguration configuration)
        ...
        public void DesignateCurrent(ISedulousWindow window, IntPtr context)
        {
            if (Current != window)
            {
        ...
        /// <inheritdoc/>
        protected override Boolean InitializeRenderingAPI(SedulousGraphicsConfiguration graphicsConfig, Int32 attempt)
        {
        ...
        /// <inheritdoc/>
        protected override void OnWindowCleanup(ISedulousWindow window)
        {
        ...
        /// <inheritdoc/>
        protected override void OnWindowCreated(ISedulousWindow window)
        {
            base.OnWindowCreated(window);
        }

        /// <inheritdoc/>
        protected override void OnWindowDestroyed(ISedulousWindow window)
        {
        ...
        /// <summary>
        /// Updates the value of the <see cref="SDL2SedulousWindow.IsCurrentWindow"/> property for the specified window.
        /// </summary>
        private void UpdateIsCurrentWindow(ISedulousWindow window, Boolean value)
        {
            var win = window as SDL2SedulousWindow;
            if (win == null)
                return;
        ...
        /// </summary>
        private void BindOpenGLContextToWindow(ISedulousWindow window, IntPtr context)
        {
            ...
            var win = (SDL2SedulousWindow)(window ?? GetMaster());
            var winptr = (IntPtr)win;
            if (SDL_GL_MakeCurrent(winptr, context) < 0)
                throw new SDL2Exception();

            if (SDL_GL_SetSwapInterval(win.SynchronizeWithVerticalRetrace ? 1 : 0) < 0 && Sedulous.Platform != SedulousPlatform.iOS)
            {
                if (!shuttingDown)
                    throw new SDL2Exception();
            }
        ...
        private SDL2SedulousWindow glwin;
